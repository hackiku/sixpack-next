...
const AppPreview = ({ isSticky, isAligned }) => (
	<div className={`relative w-[280px] mx-auto transform transition-all duration-500 
    ...
    ${isAligned ? 'mr-24' : 'mx-auto'}`}>
		<div className="relative border-4 border-black rounded-xl overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-white">
			...
			<button className="absolute inset-0 w-full h-full flex items-center justify-center bg-black bg-opacity-0 hover:bg-opacity-30 transition-opacity duration-200">
				<PlayIcon />
			</button>
		</div>
...
const HeroSection = () => {
	...
	useEffect(() => {
		const handleScroll = () => {
			if (heroRef.current && deviceSectionRef.current) {
				const heroRect = heroRef.current.getBoundingClientRect();
				const deviceRect = deviceSectionRef.current.getBoundingClientRect();

				setIsSticky(heroRect.top <= 0);
				setIsAligned(deviceRect.top <= window.innerHeight);
				setShowDeviceSection(deviceRect.top <= window.innerHeight / 2);
			}
		};
			...
			>
				<div className="p-12 flex items-center justify-center min-h-screen">
					<div className="text-center">
						...
						<p className="text-2xl mb-6">Connect your device and start exploring your flight data</p>
						<button className="px-8 py-3 bg-black text-white font-bold rounded-lg shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
							Get Started
						</button>
					</div>
				</div>
